...
- `code` is exactly what the client submitted via `POST /processes`. No
  transpilation, no wrapping — that is the environment's job.
...
queued ──► running ──► idle
   ...
   │
   └──► idle (canceled, before reaching running)
```

- `idle` — not executing. Either pre-run, or post-run with an exit
  state.
...
**environment code never returns null** — only the four listed values.
...
- Custom environments are free to use any mechanism — process group
  signals, worker thread interrupts, whatever fits the runtime — as long
  as they return `"timeout"` reliably.
...
2. Resolve the outstanding `execute(eid)` promise with `"canceled"` (or
   close to it — the host accepts any exit state).
