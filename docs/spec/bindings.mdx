---
title: "EnvironmentBindings"
description: "Callbacks an environment module receives from the host"
sidebarTitle: "EnvironmentBindings"
---

`EnvironmentBindings` is the callback surface MCI hands to an environment
module at activation. The environment calls *into* these to talk to the
host — there is no other channel back.
...
These are the calls user code ultimately drives. The environment is
expected to expose them in whatever surface the runtime uses
(`mci.discoverServices(...)`, `mci.services[id].tools[t].invoke(...)`,
etc.) and proxy them through.

| Callback | What it returns |
| --- | --- |
| `discoverServices` | Filtered service summaries (id, name, description, enabled). |
| `discoverTools` | Filtered tool summaries across services. |
| `getService` | Single service definition (no `tools`, no `adapterDomain`). |
| `getTool` | Single tool with `inputSchema` and `outputSchema`. |
| `getToolDocs` | Markdown reference for one tool — usually produced by the active environment's own `generateToolDocs`. |
| `invokeTool` | Whatever the adapter returns. Throws on disabled / missing service or tool. |

The discovery callbacks are the same calls the HTTP API uses internally,
so they apply the same `enabled` filters.
...
The host wires these in `apps/api/src/app.ts` against `ProcessService`,
`ServicesService`, and `ModuleService`. Environment modules see only
the interface — they never touch those services directly.
