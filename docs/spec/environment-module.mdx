...
own. See [Environment Modules](/content/modules/environments) for the
user-facing description.

## Setup

`setup({ bindings })` is called when the environment is activated.
`bindings` is the host's [callback surface](/spec/bindings) — store it
and use it to report execution state, capture I/O, and forward tool
discovery / invocation requests.
...
`mci.*` surface; a custom environment that exposes a different shape
...
If the environment exposes tools as `mci.services[s].tools[t].invoke(p)`,
...
See [`EnvironmentBindings`](/spec/bindings) for the callbacks and
[Execution](/spec/execution) for the state machine details.
