...
**MCI (Model Control Interface)** is an open-source system that enables **fast**, **reliable**, **secure**, and **efficient** integration between LLM applications and external services.
...
- **Code Execution Model:** Instead of writing tool calls, with MCI models write code to perform actions, cutting down tokens and allowing models to perform very long, complex tasks.
- **Asynchronous Execution**: Process executions and all manner of tool calls done with MCI allow the model to perform tasks asynchronously and poll results when they are needed, instead of all tool calls blocking the model.
