...
**MCI (Model Control Interface)** is open-source software that enables **fast**, **reliable**, **secure**, and **efficient** integration between LLM applications and APIs.

MCI lets users plug in any service over any protocol with any standard to any AI client and use it efficiently at low cost.
...
MCI, on the other hand, is neither a standard nor a new protocol... It's software that sits between the AI application and any external service, regardless of which protocol or standard that service uses. This shifts the heavy lifting from the users and developers to us.

Beyond core principles, MCI has many advantages and differences. A few of them are as follows:

- **Code Execution Model:** Instead of writing tool calls, with MCI, LLMs write code to perform actions, cutting down tokens and allowing models to perform very long, complex tasks.
- **Asynchronous Execution**: MCI lets clients start process execution without blocking other requests and retrieve process results when needed.
