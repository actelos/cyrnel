...
2. **Runtime bindings.** Exposing the `mci.*` API (or whatever shape it
...
[`EnvironmentModule`](/spec/environment-module) interface.
...
1. **`setup({ bindings })`** — called when the environment is activated.
   `bindings` is the [`EnvironmentBindings`](/spec/bindings) callback
   surface back into the host (discovery, invocation, state, output).
2. **`execute(input)`** — called for each process. `input.eid` is the
   process id; the environment must use it on every callback so the host
   can route stdout/stderr/output to the right process.
3. **`kill(eid)`** — interrupt a running or queued execution. Should
   return promptly.
4. **`generateDocs()`** — return a Markdown string describing the
   environment's globals. Surfaced by `GET /environment/docs`.
5. **`generateToolDocs(input)`** — return Markdown describing how to call a
   single tool inside this environment. Surfaced by
   `GET /tools/:serviceId/:toolId/docs`.
6. **`teardown()`** — called when the environment is deactivated.

## Active environment vs draining

Only one environment is active at a time — the one whose `modules` row has
`enabled = true`. Switching environments (or disabling the active one)
moves it to a **draining** state: no new executions are dispatched to it,
but in-flight ones run to completion. Once the last execution finishes, MCI
...
- The isolate has **no module loader, no filesystem, no network, no Node
  built-ins**. The only escape hatches are the references MCI installs.
...
See [Writing a custom module](/spec/writing-a-custom-module) for file
layout and registration. See
[`EnvironmentModule`](/spec/environment-module) for the interface and
[`EnvironmentBindings`](/spec/bindings) for the callback surface.

## Built-in

- **[`typescript-ivm`](/content/builtins/typescript-ivm)** — TypeScript /
  JavaScript runtime backed by `isolated-vm`.
