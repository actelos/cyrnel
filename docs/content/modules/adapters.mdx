...
description: "Modules that connect MCI to end services"
sidebarTitle: "Adapter Modules"
---

An **adapter module** is the part of MCI that knows how to talk to a
...
1. **Parsing definitions.** Turning whatever the user supplied at install
   ...
   into a `ServiceDefinition` MCI can store.
...
Adapter modules are the **most security-sensitive** part of MCI. They:
...
Concretely, the trust boundary is "the operator of the MCI server" decides
...
- **Treat the contents of `$MCI_DATA_DIR/modules/` as code with full host
...
- **Don't enable adapters you can't read.** "Plug-and-play" is convenient
  but not safe.
- **Don't reuse `MCI_SECRETS_KEY` across environments.** A leaked key,
  combined with a copy of `mci.db`, decrypts every secret of every service.
...
- **Persist secrets outside `ServiceState`.** All secret retention must be
  ...
  Writing them to disk silently breaks the encryption guarantee MCI is
