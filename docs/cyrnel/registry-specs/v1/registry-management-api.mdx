...
[API Reference](/cyrnel-api-reference) tab.

## Endpoints

| Method | Path | Rate limit | Purpose |
| --- | --- | --- | --- |
| `POST` | `/registries` | 10/min | Add a registry (fetches the well-known document, validates, stores). Returns the auth declaration plus eligible OAuth clients per authorization-code scheme. |
| `GET` | `/registries` | - | List registries (`{items, nextCursor, hasMore}`; `limit` ≤ 100, default 20). Each item includes `configuredSchemes`. |
| `POST` | `/registries/:id/refresh` | 10/min | Re-fetch the well-known document, stamp `lastSyncedAt`, detect auth drift. |
| `POST` | `/registries/:id/auth` | 5/min | Configure machine (non-interactive) auth material for one scheme: `{schemeName, type: apiKey|basic|bearer|oauth2(+grant: client_credentials)}` (see [Authentication](/cyrnel/registry-specs/v1/authentication)). |
| `GET` | `/registries/:id/auth` | - | Live auth declaration (`schemes`, `security`) plus configured owner-scoped credentials. |
| `DELETE` | `/registries/:id/auth` | - | Remove credentials: `?schemeName=` for one slot, omitted for all. `204`, or `404` if none. |
| `GET` | `/registries/:id/credentials` | - | List owner-scoped credentials for the registry. |
| `PUT` | `/registries/:id/credentials/:scheme/...` | 10/min | Connect a scheme (`api-key`/`basic`/`bearer`/`oauth2`), authorize (`POST .../oauth/authorize`, `POST .../oauth/code`), `DELETE` to disconnect — same shape as services/modules (see [Authentication](/cyrnel/docs/v1/authentication)). |
...
- Discovery happens first: a registry with no supported capability fails
  `400`; a bad upstream yields `400`/`502`. The response includes the
  fetched `auth` declaration and `resolvedClients` per
  authorization-code scheme for instant Sign-in buttons.
- A conflicting id or base URL yields `409`.
- Credentials are connected afterwards: machine material via
  `POST /registries/:id/auth`, user-delegated OAuth via the nested
  credential endpoints. Auth is validated against the live advertisement
