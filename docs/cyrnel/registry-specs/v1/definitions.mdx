...
The `definitions.v1` capability distributes **service definitions**. An adapter
generates a service (with tools) from it. See [Definition kinds](/cyrnel/sdk-spec/5.0.0/definition-kinds)
and [Adapter modules](/cyrnel/docs/v1/adapters-modules) for what Cyrnel can
ingest.
...
Pagination is covered in [Pagination](/cyrnel/registry-specs/v1/pagination).
...
1. Cyrnel GETs the descriptor, picks the version, and (unless the request
   ...
   the [compatibility ranking](/cyrnel/registry-specs/v1/install-and-update).
