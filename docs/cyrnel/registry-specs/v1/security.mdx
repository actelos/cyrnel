...
- Install malicious modules. Module installs are subject to the same
  ...
  [Security](/cyrnel/docs/v1/security).
...
| DNS | Hostnames are resolved (`dns.lookup`) and **every** address is checked pre-fetch, then filtered again at dial time via a lookup hook so a rebind between check and connect cannot slip through (IP literals skip the hook and are validated as URLs) |
| Redirects | Max 5 on registry/download fetches; each hop re-guarded and re-authed (auth recomputed per hop, never forwarded cross-origin); `304`, missing `Location`, IP-literal / non-http(s) targets fail. OAuth2 token endpoints reject redirects outright (manual mode + 3xx rejection) since secrets live in the POST body |
...
`CYRNEL_REGISTRY_AUTH_INSECURE_CIDRS` (enforced pre-fetch and at dial time
via a DNS lookup filter). Credentials attach only within the
registry's origin + base-path scope and are recomputed per hop, never
forwarded cross-origin — including custom apiKey headers, which HTTP clients
do not strip on their own (only `authorization`/`cookie` are stripped by
default). See [Authentication](/cyrnel/registry-specs/v1/authentication).

Residual risk: a CONNECT proxy resolves origin DNS proxy-side, where no
client-side lookup hook fires. No proxy is configured for API egress, so the
https-required rule carries that path; re-evaluate if proxy egress is added.
...
[Secrets](/cyrnel/docs/v1/security#secrets).
