...
| URL | Purpose | See |
| --- | --- | --- |
| `GET /.well-known/registry.json` | Discovery + capability advertisement | [Well-known](/cyrnel/registry-specs/v1/well-known) |
| `GET <capability URL>` | Browse page for definitions or modules | [Definitions](/cyrnel/registry-specs/v1/definitions), [Modules](/cyrnel/registry-specs/v1/modules) |
| `GET <entry.source>` | Version descriptor for one item | [Definitions](/cyrnel/registry-specs/v1/definitions) / [Modules](/cyrnel/registry-specs/v1/modules) |
| `GET <version.downloadUrl>` | The artifact (definition document or module archive) | [Definitions](/cyrnel/registry-specs/v1/definitions) / [Modules](/cyrnel/registry-specs/v1/modules) |
| `POST <tokenEndpoint>` | Optional OAuth2 token exchange | [Authentication](/cyrnel/registry-specs/v1/authentication) |
...
| --- | --- | --- |
| `CYRNEL_DEV_REGISTRY_AUTH_MODE` | `none` | `none`, `apikey`, `basic`, `bearer`, `oauth2` (client-credentials), `oauth2-ac` (authorization-code, auto-approving), or `multi` (apiKey + oauth2-cc with a public-definitions override). `none` advertises no auth key. |
| `CYRNEL_DEV_REGISTRY_API_KEY` | `dev-registry-key` | apikey/multi mode: expected `x-dev-registry-key` header value |
| `CYRNEL_DEV_REGISTRY_BASIC_USER` / `CYRNEL_DEV_REGISTRY_BASIC_PASS` | `dev` / `devpass` | basic mode: expected Basic credentials |
| `CYRNEL_DEV_REGISTRY_TOKEN` | `dev-registry-token` | bearer/oauth2/oauth2-ac mode: issued bearer token |
| `CYRNEL_DEV_REGISTRY_CLIENT_ID` | `dev-client` | oauth2/oauth2-ac mode: accepted `client_id` |
| `CYRNEL_DEV_REGISTRY_CLIENT_SECRET` | `dev-secret` | oauth2/oauth2-ac mode: accepted `client_secret` |
| `CYRNEL_DEV_REGISTRY_TOKEN_EXPIRES_IN` | `3600` | oauth2/oauth2-ac mode: tokens' `expires_in` |
| `CYRNEL_DEV_REGISTRY_DRIFT_AUTH` | `0` | `1` advertises auth endpoint values that differ from what the server enforces ("drift") |
...
<Warning>
  ...
  [entry schema](/cyrnel/registry-specs/v1/definitions)) to browse
  definitions cleanly.
</Warning>
