...
| Auth | Configured scheme slots (per-scheme status + granted scopes) and token expiry for oauth2 |
...
- **Base URL**: any absolute `http(s)` URL. Cyrnel fetches
  ...
  identifier. The response includes the advertised `auth` declaration and
  `resolvedClients` so per-scheme Sign in renders immediately.
- **Auth** (optional, per scheme): API key, basic, bearer, or OAuth2
  credentials, claimed per declared scheme. Auth is only usable when the
  registry advertises it; see the
  [Registry Specification](/cyrnel/registry-specs/v1/registry-overview)
  and [Authentication](/cyrnel/docs/v1/authentication).

### Update & remove

- **Refresh** re-fetches the well-known document (stamps "last synced")
  and detects auth drift.
- **Delete** removes the registry and cascades its owned credentials.

### Authenticate a registry

Each scheme the registry's well-known `auth` declares has one slot
holding at most one credential, claimed through the same nested routes
as services (`PUT /registries/:id/credentials/:scheme/api-key | /basic
| /bearer | /oauth2`, `POST .../oauth/authorize`, `POST .../oauth/code`,
`DELETE .../:scheme`). `oauth2` schemes with an `authorization_code`
grant resolve against shared OAuth clients (Sign in); schemes with a
`client_credentials` grant hold a per-registry machine secret. See
[Authentication](/cyrnel/registry-specs/v1/authentication) and the
[operator guide](/cyrnel/docs/v1/authentication).
...
See [Install & update](/cyrnel/registry-specs/v1/install-and-update) for the
full flows.
...
See [Registry security](/cyrnel/registry-specs/v1/security) for the complete
