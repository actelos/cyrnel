...
- **[Adapter modules](/cyrnel/docs/v1/adapters-modules)** translate environment
  invocations into calls to end services.
- **[Environment modules](/cyrnel/docs/v1/environments-modules)** execute the code
  clients submit.
...
2. **Custom modules**, loaded from `$CYRNEL_DATA_DIR/modules/<name>/`. Each
   directory must contain a `module.json`:
    ...
    Each entry declares an `<identifier>` and a semver `version` range that the
    adapter can consume (see [Definition kinds & adapter
    compatibility](/cyrnel/sdk-spec/5.0.0/definition-kinds)). When a service definition
    carries a matching kind, the adapter is offered as an install target for it.

   `main` is an ES module exporting `instantiate(): AdapterModule` or
   `EnvironmentModule`. See
   [Writing a custom module](/cyrnel/sdk-spec/5.0.0/writing-custom-modules).
...
content is re-fed through the new adapter's `generateService`. Services
...
not in `module.json`. Modules that need end-service credentials additionally
declare `auth` (`{ schemes, security }`) on the same export, persisted to
`modules.auth_schemes` / `modules.default_security` for credential validation.
Cyrnel stores module configuration as plaintext JSON
...
| --- | --- | --- | --- |
| `GET` | `/modules` | -- | Filterable by `query`, `type`, `isBuiltin`, `enabled`, `limit`, `cursor` |
...
`GET /modules` returns the paginated envelope `{ items, nextCursor, hasMore }`; see [Pagination](/cyrnel/docs/v1/pagination) for the cursor rules, `limit` semantics, and error codes.
...
interface ModuleManifestRecord {
  ...
  credentialSchemes?: Record<schemeName, { configured: boolean; status?: string; grantedSource?: string | null }>;
}
```

`schemes` / `security` mirror the module export's `auth` declaration
(`{}` / `[]` when the module declares no auth); `credentialSchemes` is
present when at least one scheme slot holds a credential. Like
services, each scheme holds at most one owned credential
(`PUT /modules/:moduleId/credentials/:scheme/...`), and owned
credentials resolve at setup through `credentials.getCredential()` —
including the OAuth authorize flow. See [Authentication](/cyrnel/docs/v1/authentication).
