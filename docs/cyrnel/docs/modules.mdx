...
2. **Custom modules**, loaded from `$CYRNEL_DATA_DIR/modules/<name>/`. Each
   directory must contain a `module.json`:
    ...
    compatibility](/cyrnel/module-specs/definition-kinds)). When a service definition
   ...
   [Writing a custom module](/cyrnel/module-specs/writing-custom-modules).
...
`GET /modules` returns the paginated envelope `{ items, nextCursor, hasMore }`; see [Pagination](/docs/pagination) for the cursor rules, `limit` semantics, and error codes.
