...
targets this adapter: for each non-orphaned service, the stored definition
...
Same service regeneration behavior as the direct update: every non-orphaned
...
Enabling an orphaned module returns `409`. Direct-installed items (`source = ""`)
...
- `install_module` (registry), `add_module` (direct), `update_module` (registry),
  `patch_module` (direct), `delete_module`
