...
cyrnel creates a process with a unique process id (`pid`), runs it on the active
[environment module](/cyrnel/docs/environments-modules), captures its output, and
...
If `block: true`, cyrnel polls until `state === "idle"` before responding. The
HTTP status is still `201`. Note that `block` has no effect when `autorun` is
`false`, the process is already idle.
