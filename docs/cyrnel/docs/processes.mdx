...
All list endpoints return the paginated envelope
`{ items, nextCursor, hasMore }`. Pass the previous response's `nextCursor`
back as the `cursor` query parameter to fetch the next page; `nextCursor` is
`null` when there are no more items. `limit` is 1–100 (default 20, values
above 100 are clamped). A malformed or expired cursor returns `400` with a
stable error code (`invalid_cursor` or `cursor_expired`). Cursors encode
position only — always restart from the first page when the filters change.
