...
Adapter modules implement the SDK [`AdapterModule`](/specs/adapter-module)
interface.
...
2. **`generateDefinition(content)`** is called during `POST /services/install`
...
See [Writing a custom adapter module](/specs/writing-custom-adapter-module)
for the file layout, a skeleton, and a full working example.
[`AdapterModule`](/specs/adapter-module) covers the interface contract
in full.
