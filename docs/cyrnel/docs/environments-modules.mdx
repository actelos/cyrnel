...
3. **Output capture.** Streaming stdout, stderr, and structured output back
 ...
 4. **Cancellation and timeouts.** Honouring `kill(eid)` and respecting
   `input.timeoutMs` (API-level) and `input.envConfig.timeoutMs`
...
1. **`setup({ bindings })`** is called when the environment is activated.
 ...
 2. **`execute(input)`** is called for each process. `input.eid` is the
   ...
   can route stdout/stderr/output to the right process. `input.timeoutMs`
   is the API-level deadline; `input.envConfig` is a
   `Record<string, unknown>` of environment-specific configuration
   forwarded from the request. Each environment module documents its
   accepted `envConfig` keys.
...
`input.timeoutMs` (set via the API-level `timeoutMs` field on
`POST /processes`) is the hard deadline for the execution, the host will
call `kill(eid)` if it elapses. `input.envConfig.timeoutMs` is an
optional environment-specific override that the environment module can
use for finer-grained control (e.g. the TypeScript isolate's transpile
step). The two levels are independent; the shorter of the two always
wins.
