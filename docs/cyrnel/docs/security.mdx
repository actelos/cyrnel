...
- **There is no domain allowlist.** The guard only checks IP ranges, not
  ...
  addresses will pass.
- **Bypass on failure.** If DNS fails to resolve, the request is rejected
  with `502`. If `CYRNEL_ALLOW_PRIVATE_REGISTRY=1` (or `true`) is set,
  the guard is skipped entirely, use this for development only.
...
- If you need a stricter posture (e.g. a domain allowlist or egress
  filtering), run cyrnel inside a network namespace or egress firewall
  that restricts which hosts it can reach.
