...
## Service and Module Installation (Remote Fetching)
...
to the adapter's `generateDefinition`.

Module installation and update routes use the same guarded download path
for module archives and registry metadata: `POST /modules` (direct
install), `POST /modules/install` (registry install),
`PATCH /modules/:moduleId` (direct update), and
`POST /modules/:moduleId/update` (registry update).

The shared fetcher applies the following guards to both service and module
downloads:
...
- Treat the API key as the security boundary. Do not expose service or
  module installation endpoints (or the API at all) to clients you
  wouldn't trust to make outbound network requests from the host.
