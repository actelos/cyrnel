...
- **Authentication is one static bearer token** (`CYRNEL_API_KEY`). No scopes,
  ...
  See the [README](https://github.com/actelos/cyrnel#talk-to-the-api).
...
- **DNS names are not resolved or pinned.** A caller can hand cyrnel any
...
- If you need a stricter posture, run cyrnel inside a network namespace or
...
- Don't deploy the web UI publicly with credentials baked into the
  bundle.
- Review adapters before dropping them into `$CYRNEL_DATA_DIR/modules/`.
