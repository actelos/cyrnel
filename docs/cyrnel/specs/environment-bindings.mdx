...
| --- | --- |
| `discoverServices` | Filtered service summaries (id, name, description, enabled, stale). |
| `discoverTools` | Filtered tool summaries across services. |
| `getService` | Single service definition (no `tools`, no `adapterDomain`; includes `stale` and `effectivelyEnabled`). |
| `getTool` | Single tool with `inputSchema` and `outputSchema` (includes `effectivelyEnabled`). |
...
so they apply the same `enabled` and `stale` filters.
