...
[`ServiceDefinition`](/specs/types#servicedefinition).

- Called from `POST /services` (direct install), `POST /services/install`
  (registry install), `PATCH /services/:serviceId` (direct update), and
  `POST /services/:serviceId/update` (registry update).
