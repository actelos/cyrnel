...
For the full interface reference, see [`AdapterModule`](/cyrnel/sdk-spec/5.0.0/adapter-module)
and [`EnvironmentModule`](/cyrnel/sdk-spec/5.0.0/environment-module).
...
For adapters, enabling triggers `setup({ config, secrets, logger, credentials })`
where `config`/`secrets` are scope-bound providers (`get(key)` only) and
`credentials` is present iff the module export declares `auth`. For environments, enabling makes it
...
export default {
  ...
  auth: { /* optional AuthDefinition: { schemes, security } */ },
  // environments only:
  // executionConfigSchema: { /* JSON Schema for per-execution envConfig */ },
...
[Types → Module logging](/cyrnel/sdk-spec/5.0.0/types#module-logging) and the
