...
def list_processes(
    state: Annotated[
        ...
        Field(
            description=(
                "Optional process state filter. One of: idle, queued, running, "
                'terminating. Example: "idle".'
            )
        ),
    ] = None,
    status: Annotated[
        ...
        Field(
            description=(
                "Optional process status filter. One of: success, failed, timeout, "
                'canceled, null. Use "null" to filter processes with no terminal '
                'status yet. Example: "success".'
            )
        ),
    ] = None,
    ref: Annotated[
        ...
        Field(
            description=(
                "Optional reference label filter (exact match after trimming). "
    ...
    """
    List stored processes using optional state/status/ref filters, returning process
    records.
    ...
    Returns:
        Object with key `processes` (list of processes). Returns `processes=[]` when no
        matches.
    ...
    Example:
        ...
        → {"processes": [
            {"pid": 12, "ref": "nightly-sync", "state": "idle", "status": null}
          ]}
    """
    params: Dict[str, Any] = {}
...
def get_process(
    ...
    Example:
        get_process(12) →
        {"pid": 12, "ref": "demo", "state": "idle", "status": "success"}
    """
...
def create_process(
    ...
    A process encapsulates runnable code executed by the MCI environment. If `block` is
    true, the API waits until the process returns to the `idle` state before responding.

    When to use:
        - Use to execute code that discovers services/tools or invokes them.
    When NOT to use:
        - If you want to re-run an existing idle process, use
          `run_process(pid, force=...)`.
...
def delete_process(
    ...
    When NOT to use:
        - If the process is running or queued, call `kill_process(pid)` first, then
          delete.
...
def get_process_code(
    ...
    Returns:
        Raw code string (text/plain). Returns an empty string only if the stored
        code is empty.
...
def get_process_stdout(
    ...
    Returns:
        Raw stdout string (text/plain). Returns an empty string if the process
        produced no stdout.
...
def get_process_stderr(
    ...
    Returns:
        Raw stderr string (text/plain). Returns an empty string if the process
        produced no stderr.
...
def run_process(
    ...
    force: Annotated[
        ...
        Field(
            description=(
                "Whether to overwrite existing outputs before rerunning. Example: "
                "false."
            )
        ),
    ] = False,
    ...
    The API only accepts a run signal when the process is currently `idle`. If `force`
    is false and the process has existing outputs, the API rejects the request.
    ...
    Example:
        run_process(12, force=True, block=True) →
        {"pid": 12, "state": "idle", "status": "success"}
    """
    return _request(
        "POST",
...
def kill_process(
    ...
    Returns:
        Updated process object. If the process was queued, it may transition
        to `idle` with status `canceled`.
...
def discover_services(
    query: Annotated[
        ...
        Field(
            description=(
                "Optional search string to match service name/description. Whitespace "
                'is trimmed. Example: "github".'
            )
        ),
    ] = None,
    limit: Annotated[
    ...
    enabled: Annotated[
        ...
        Field(
            description=(
                ...
                "Note: omitting this parameter uses the API default"
                "(commonly enabled-only)."
            )
        ),
    ] = None,
) -> DiscoverServicesResponse:
    """
    Discover installed services by query and filters, returning matching service
    summaries.

    This tool is read-only: it searches the manifest store and returns metadata for
    each match.

    When to use:
        - Use when you want to shortlist services by name/description.
    When NOT to use:
        - If you already know the exact service name, call `get_service(service_name)`
          instead.
    ...
    Returns:
        Object with key `services` (list). Returns `services=[]` if there are no
        matches.
...
def get_service(
    ...
    Returns:
        Service details object with keys: `name`, `type`, `source`, `description`,
        `hash`, `enabled`, `configSchema`, `secretsSchema`.
...
def get_service_configuration_schema(
    ...
    When NOT to use:
        - If you need current configured values, call `get_service_configuration`
          instead.
    ...
    Example:
        get_service_configuration_schema("github") → {
...
def get_service_configuration(
    ...
    When NOT to use:
        - If you need the allowed shape/constraints, call
          `get_service_configuration_schema` instead.
    ...
    Example:
        get_service_configuration("github") →
...
def patch_service_configuration(
    ...
    patch: Annotated[
        ...
        Field(
            description=(
                "JSON Patch operation list to apply to the service configuration"
                "(RFC 6902 style). "
    ...
    """
    Patch a service configuration with JSON Patch operations, returning the updated
    `config`.

    This mutates stored configuration and may trigger downstream restaging behavior in
    the API.
    ...
    Args:
        service_name: Exact service identifier (e.g. "github").
        patch: JSON Patch operation list
               (e.g. [{"op":"replace","path":"/timeout","value":30}]).
    ...
    Example:
        patch_service_configuration(
            \"github\", patch=[{\"op\":\"replace\",\"path\":\"/timeout\",\"value\":30}]
        )
        → {"config": {"timeout": 30}}
    """
    return _request(
...
def get_service_secrets_schema(
    ...
    Example:
        get_service_secrets_schema("github") → {
...
def patch_service_secrets(
    ...
    patch: Annotated[
        ...
        Field(
            description=(
                "JSON Patch operation list to apply to the service secrets payload "
                "(RFC 6902 style). "
    ...
    Args:
        service_name: Exact service identifier (e.g. "github").
        patch: JSON Patch operation list
               (e.g. [{"op":"add","path":"/token","value":"ghp_..."}]).
    ...
    Example:
        patch_service_secrets(
            "github",
            patch=[{"op": "add", "path": "/token", "value": "ghp_123"}],
        )
        → {"updated": true}
    """
    return _request(
...
def create_service(
    ...
    """
    Install a service manifest from a registry source, returning the installed service
    name/type.

    This fetches the manifest definition from `source` and stores it in the API's
    manifest database.

    When to use:
        - Use to add a new service from a known manifest URL.
    When NOT to use:
        - If the service is already installed and you want to refresh it, use
          `update_service`.

    Args:
        service_type: Registry type identifier (e.g. "registry").
        source: Manifest definition URL
                (e.g. "https://registry.example.com/service.json").
...
def update_service(
    ...
    """
    Refresh a service manifest from its stored install source, returning whether
    it changed.
    ...
    Returns:
        Object with keys: `name` and `updated` (boolean). `updated=false`
        means no changes were applied.
...
def set_service_enabled(
    ...
    Example:
        set_service_enabled(\"github\", enabled=True) →
...
def delete_service(
    service_name: Annotated[
        ...
        Field(
            description=(
                'Exact service identifier (manifest id) to delete. Example: "github".'
            ),
            min_length=1,
        ),
    ],
...
def discover_tools(
    ...
    enabled: Annotated[
        ...
        Field(
            description=(
                "Optional enabled filter. Example: true. Note: omitting this parameter "
                "uses the API default (commonly enabled-only)."
            )
        ),
    ] = None,
) -> DiscoverToolsResponse:
    """
    Discover tools across services by query and filters, returning matching tool
    summaries.
    ...
    When NOT to use:
        - If you already know the exact service+tool name, call
          `get_tool(tool_name, service_name=...)` instead.
...
def get_tool(
    tool_name: Annotated[
        ...
        Field(
            description=(
                'Tool name identifier (within the service). Example: "listIssues".'
            ),
            min_length=1,
        ),
    ],
    ...
    """
    Fetch a tool definition by name (and optional service), returning schemas and
    enabled status.
    ...
    Returns:
        Tool details object with keys: `name`, `description`, `enabled`,
...
def set_tool_enabled(
    ...
    When NOT to use:
        - If you need to enable/disable the whole service, use `set_service_enabled`
          instead.
